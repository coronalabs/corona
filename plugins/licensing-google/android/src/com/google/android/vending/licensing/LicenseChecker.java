...
public class LicenseChecker implements ServiceConnection {
     ...
     * @param callback
     */
    public synchronized void checkAccess(LicenseCheckerCallback callback, boolean force) {
        // If we have a valid recent LICENSED response, we can skip asking Market.
        // If we have an older version code then we still want to check so we can download any potential expansion/patch files
        if (mPolicy.allowAccess() && !CoronaProvider.licensing.google.LuaLoader.isNewAppVersion() && !force) {
