...
Options:
  ...
  -n, -names      Flag that indicates provided input names are actually package
                  ...
                  the path defined by -luapath ption. This flag can be used in
...
local outc = assert(io.open(filepath..".cpp", "w"))
-- local outh = assert(io.open(filepath..".h", "w"))

local guard = ""
if outh then
guard = output .. "_H"
guard = guard:upper():gsub("[^%w]", "_")
...
for i, input in ipairs(inputs) do
	...
	-- Suppresses an Xcode compiler warning
	outc:write("extern ", prefix," int luaload_",modulename,func,"(lua_State *L);\n")
	outc:write("extern ", prefix," const unsigned char *Rtt_GetLuaByteCode_",modulename,func,"(size_t *len);\n\n")

	if outh then
		outh:write(prefix," int luaload_",modulename,func,"(lua_State *L);\n")
		outh:write("extern ", prefix," const unsigned char *Rtt_GetLuaByteCode_",modulename,func,"(size_t *len);\n\n")
	end
	outc:write(
prefix,[[ const unsigned char *
Rtt_GetLuaByteCode_]],modulename,func,[[(size_t *len)
{
	*len = sizeof(B]],i,[[);
	return B]],i,[[;
}
...
// ----------------------------------------------------------------------------
	...
	outc:write(
prefix,[[ int
luaload_]],modulename,func,[[(lua_State *L)
{
	return luaL_loadbuffer(L,(const char*)B]],i,[[,sizeof(B]],i,[[),"]],input,[[");
}
