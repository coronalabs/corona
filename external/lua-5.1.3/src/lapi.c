...
static const char *aux_upvalue (StkId fi, int n, TValue **val) {
  ...
  if (f->c.isC) {
    ...
    *val = &f->c.upvalue[n-1];
    ...
    // This code was modified from original lua 5.1 code
    // It appears that debug stripping also nukes whole p->upvalues array
    // not just names, so without names all upvalue requests fail
    // More http://lua.2524044.n2.nabble.com/lua-g-s-etupvalue-and-debug-stripping-td7681605.html
    // original code:  https://www.lua.org/source/5.1/lapi.c.html#aux_upvalue
    Proto *p = f->l.p;
    if (!(1 <= n && n <= f->l.nupvalues)) return NULL;
    *val = f->l.upvals[n-1]->v;
    return (n<=p->sizeupvalues)?getstr(p->upvalues[n-1]):"(*no name*)";
  }
}
