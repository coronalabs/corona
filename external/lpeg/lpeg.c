...
static Stack *doublestack (lua_State *L, Stack **stacklimit, int ptop) {
  ...
  Stack *newstack;
  int n = (int) (*stacklimit - stack);
  int max, newn;
  lua_getfield(L, LUA_REGISTRYINDEX, MAXSTACKIDX);
  max = (int) lua_tointeger(L, -1);
  lua_pop(L, 1);
  if (n >= max)
...
static Instruction *fix_l (lua_State *L, int t) {
  ...
  while (lua_next(L, t) != 0) {
    ...
    l = (int) pattsize(L, -1) + 1;  /* space for pattern + ret */
  ...
  for (i = 1; i <= n; i++) {  /* check all rules */
    int l = (int) pattsize(L, base + 1 + i*2) + 1;
...
static Instruction *getpatt (lua_State *L, int idx, int *size) {
  ...
  if (size) *size = (int) pattsize(L, idx);
  return p;
}
