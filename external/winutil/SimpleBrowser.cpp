...
SimpleBrowser::SimpleBrowser()
	:	_Browser(NULL),
		...
		_Ready(false),
		_Content(_T("")),
		_IsMessageTranslationEnabled(true)
{
}
...
bool SimpleBrowser::GetRegisterAsDropTarget()
{
	bool isRegistered = false;
	if (_Browser)
	{
		VARIANT_BOOL value = VARIANT_FALSE;
		_Browser->get_RegisterAsDropTarget(&value);
		isRegistered = (VARIANT_TRUE == value);
	}
	return isRegistered;
}

void SimpleBrowser::PutRegisterAsDropTarget(bool value)
{
	if (_Browser)
	{
		_Browser->put_RegisterAsDropTarget(value ? VARIANT_TRUE : VARIANT_FALSE);
	}
}

bool SimpleBrowser::IsMessageTranslationEnabled()
{
	return _IsMessageTranslationEnabled;
}

void SimpleBrowser::SetMessageTranslationEnabled(bool value)
{
	_IsMessageTranslationEnabled = value;
}
...
BOOL SimpleBrowser::PreTranslateMessage(MSG *pMsg)
{
	...
	if (!_IsMessageTranslationEnabled) {
		// Corona Labs:
		// Do not allow Internet Explorer style shortcut keys or right click menus to work.
		switch (pMsg->message) {
			case WM_RBUTTONDOWN:
		...
		// Let the IE web browser control handle the received messages.
		...
		// Let the container control do its default handling.
		result = CWnd::PreTranslateMessage(pMsg);
	}
