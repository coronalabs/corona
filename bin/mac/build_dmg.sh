...
(
	...
	(
		...
		codesign --timestamp --deep --force --options runtime --strict --sign 'Apple Distribution' "${LIBRARIES[@]}"
		codesign --timestamp --deep --force --options runtime --strict --sign 'Apple Distribution' --entitlements "$ENTITLEMENTS"  "${EXECUTABLES[@]}"
	)
	}
...
codesign --timestamp --deep --force --options runtime --strict --sign "Apple Distribution" "$DSTBASE/$DMG_FILE"
