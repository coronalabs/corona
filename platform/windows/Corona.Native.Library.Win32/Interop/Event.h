...
class Event : public BaseEvent
{
	public:
		...
		{
			public:
				virtual void Invoke(TSender sender, TEventArgs arguments) const = 0;
		};
		...
		class FunctionHandler : public Handler
		{
			public:
				...
				virtual void Invoke(TSender sender, TEventArgs arguments) const
				{
					if (fFunctionPointer)
					{
		...
		class HandlerManager
		{
			public:
				...
				/// </param>
				void Add(const Handler *handlerPointer)
				{
				...
				/// </returns>
				bool Contains(const Handler *handlerPointer) const
				{
					if (handlerPointer)
					{
						std::lock_guard<std::recursive_mutex> scopedMutexLock(fEventPointer->fMainMutex);
		...
		void Raise(TSender sender, TEventArgs arguments)
		{
			...
			const Handler *handlerPointerStackArray[kMaxStackArraySize];
			const Handler **handlerPointerHeapArray = nullptr;
			const Handler **handlerPointerArray = nullptr;
			int handlerCount = 0;
			{
				...
				{
					handlerPointerHeapArray = new const Handler*[handlerCount];
	...
	private:
		typedef std::vector<const Handler*> HandlerCollection;
