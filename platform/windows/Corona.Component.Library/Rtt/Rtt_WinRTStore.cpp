...
WinRTStore::WinRTStore(const ResourceHandle<lua_State>& handle)
:	PlatformStore(handle)
{
}

/// Destructor. Cleans up allocated resources.
WinRTStore::~WinRTStore()
{
}
...
void WinRTStore::UseStore(const char *storeName)
{
}

/// Enables store transaction events to be sent to the Lua listener.
void WinRTStore::StartObservingTransactions()
{
}

/// Disables store transaction events from being sent to the Lua listener.
void WinRTStore::StopObservingTransactions()
{
}

/// Fetches all purchased products from the store.
/// Note: This is not currently supported on WinRT.
void WinRTStore::LoadProducts(lua_State *L, int index)
{
}
...
void WinRTStore::Purchase(lua_State *L, int index)
{
	// Validate.
	if (!L || (index < 0))
	{
...
bool WinRTStore::CanMakePurchases() const
{
	return true;
}
...
///       Will remain this way until we have an WinRT in-app purchasing system that supports it.
...
bool WinRTStore::CanLoadProducts() const
{
	return false;
}
...
void WinRTStore::FinishTransaction(PlatformStoreTransaction *transaction)
{
	// Validate argument.
	if (!transaction)
	{
...
void WinRTStore::RestoreCompletedTransactions()
{
}
