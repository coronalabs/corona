...
// TODO: Move tvosPackageApp.lua out of librtt and into rtt_player in XCode
...
// tvosPackageApp.lua is pre-compiled into bytecodes and then placed in a byte array
...
TVOSAppPackager::TVOSAppPackager( const MPlatformServices& services, MacSimulatorServices *simulatorServices /* = NULL */)
	: PlatformAppPackager( services, TargetDevice::kTVOSPlatform ),
...
exit_gracefully:
	...
	{
		Rtt_LogException("tvOS build failed (%d) after %ld seconds", result, (time(NULL) - startTime));
	}
...
bool
TVOSAppPackager::VerifyConfiguration() const
{
...
char* 
TVOSAppPackager::Prepackage( AppPackagerParams * params, const char* tmpDir )
{
	char* result = NULL;
...
bool
TVOSAppPackager::CopyProvisionFile( const AppPackagerParams * params, const char* tmpDir )
{
