...
namespace Rtt
{
	...
	PlatformWebAudioPlayer sdlAudio;
	PlatformWebAudioPlayer* getSDLAudio() { return &sdlAudio; }
...
//		else
//		{
			play_channel = sdlAudio.PlayChannelTimed(which_channel, sound_data, number_of_loops, expire_ticks, callback, fade_ticks);
//		}
	...
	static int	setMaxVolume(lua_State *L)
	{
		/**
		...
		getMaxVolume(lua_State *L)
	{
		...
		if ((1 == number_of_arguments) && lua_istable(L, 1))
		{
			...
			if ((!lua_isnil(L, -1)) && (lua_isnumber(L, -1)))
			{
				al_source = (int)lua_tointeger(L, -1);
				// Source will override channel if there is an inconsistency
				which_channel = al_source; //sdlAudio.GetChannelFromSource(al_source);
			}
			lua_pop(L, 1);
		}
...
//			}
	...
	static int	supportsSessionProperty(lua_State *L)
	{
	...
	static int setProperty(lua_State *L)
	{
		...
		if (number_of_arguments >= 3)
		{
			...
			sdlAudio.setProperty(ch - 1, key, val);		// ch is 0-based
		}
		return 0;
	}


#endif // Rtt_USE_WEBMIXER
