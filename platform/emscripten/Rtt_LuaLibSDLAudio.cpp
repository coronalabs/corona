...
namespace Rtt
{
...
#ifdef Rtt_USE_SDLMIXER

	PlatformSDLAudioPlayer sdlAudio;
	PlatformSDLAudioPlayer* getSDLAudio() { return &sdlAudio; }

	static int		loadSoundAll(lua_State *L)
	{
		/**
		...
		if (lua_isstring(L, 1))
		{
			...
			if (lua_isnumber(L, current_stack_argument))
			{
			...
			sound_data = sdlAudio.LoadAll(filePath.GetString());
			if (sound_data >= 0)
			{
		...
		loadSoundStream(lua_State *L)
	{
		...
		if (lua_isstring(L, 1))
		{
			...
			// Get the full path after we parse for the baseDir parameter
			platform.PathForFile(filename, baseDir, MPlatform::kDefaultPathFlags, filePath);
			sound_data = sdlAudio.LoadStream(filePath.GetString(), buffer_size, max_queue_buffers, num_startup_buffers, num_buffers_to_queue_per_update_pass);
			if (sound_data >= 0)
			{
				lua_pushinteger(L, sound_data);
			}
	...
	{
		/**
		 @fn freeData(SDLmixer_Data* SDLmixer_data)
		 ...
		 @param SDLmixer_data The handle of the data you want to release.
		 @code
		 backgroundMusic = audio.loadStream("backgroundMusic.m4a")
		...
		if (lua_isnumber(L, 1))
		{
			int sound_data = lua_tointeger(L, 1);
			sdlAudio.FreeData(sound_data);
		}
		return 0;
	}
	...
	{
		/**
		 @fn play(SDLmixer_Data* file_handle, lua_Table optional_parameters)
		...
		if (lua_istable(L, 2))
		{
			...
			if ((!lua_isnil(L, -1)) && (lua_isnumber(L, -1)))
			{
				which_channel = (int)lua_tointeger(L, -1) - 1; // subtract 1 to convert from Lua arrays start at 1 to SDLmixer/C-array starts at 0
			}
			lua_pop(L, 1);
			...
			if ((!lua_isnil(L, -1)) && (lua_isnumber(L, -1)))
			{
				...
				which_channel = sdlAudio.GetChannelFromSource(al_source);
			}
			lua_pop(L, 1);
		...
		PlatformSDLmixerPlaybackFinishedCallback *callback = Rtt_NEW(LuaContext::GetAllocator(L), PlatformSDLmixerPlaybackFinishedCallback(context->LuaState()));
		...
		if (fade_ticks > 0)
		{
			play_channel = sdlAudio.FadeInChannelTimed(which_channel, sound_data, number_of_loops, fade_ticks, expire_ticks, callback);
		}
		else
		{
			play_channel = sdlAudio.PlayChannelTimed(which_channel, sound_data, number_of_loops, expire_ticks, callback);
		}
		...
		{
			play_source = sdlAudio.GetSourceFromChannel(play_channel);
		}

		play_channel += 1; // add 1 to convert from SDLmixer/C-array starts at 0 to Lua array starts at 1
		...
		pauseChannel(lua_State *L)
	{
		...
		int number_of_channels = sdlAudio.PauseChannel(which_channel);
		// -1 is an error condition. Since the Lua API starts with channel 1 instead of 0, we can return 0.
		if (number_of_channels < 0)
		{
			number_of_channels = 0; // add 1 to convert from SDLmixer/C-array starts at 0 to Lua arrays start at 1
		}
		...
		resumeChannel(lua_State *L)
	{
		...
		int number_of_channels = sdlAudio.ResumeChannel(which_channel);
		// -1 is an error condition. Since the Lua API starts with channel 1 instead of 0, we can return 0.
		if (number_of_channels < 0)
		{
			number_of_channels = 0; // add 1 to convert from SDLmixer/C-array starts at 0 to Lua arrays start at 1
		...
		stopChannel(lua_State *L)
	{
		...
		if (number_of_arguments > 0)
		{
			if (lua_istable(L, 1))
			{
				...
				if ((!lua_isnil(L, -1)) && (lua_isnumber(L, -1)))
				{
					...
					which_channel = which_channel - 1; // subtract 1 to convert from Lua arrays start at 1 to SDLmixer/C-array starts at 0;
				}
				lua_pop(L, 1);
				...
				if ((!lua_isnil(L, -1)) && (lua_isnumber(L, -1)))
				{
					al_source = (int)lua_tointeger(L, -1);
					// Source will override channel if there is an inconsistency
					which_channel = sdlAudio.GetChannelFromSource(al_source); // In SDLmixer/C channel ranges
				}
				lua_pop(L, 1);
				...
				// Note we compare using '<' and not '<=' because 1 is already subtracted from the above and we are already in SDLmixer channel ranges.
				if (which_channel < 0)
				{
		...
		int number_of_channels = sdlAudio.StopChannel(which_channel);
		// -1 is an error condition. Since the Lua API starts with channel 1 instead of 0, we can return 0.
		if (number_of_channels < 0)
		{
			number_of_channels = 0; // add 1 to convert from SDLmixer/C-array starts at 0 to Lua arrays start at 1
		...
		stopWithDelay(lua_State *L)
	{
		...
		int number_of_channels = sdlAudio.ExpireChannel(which_channel, number_of_milliseconds);
		// -1 is an error condition. Since the Lua API starts with channel 1 instead of 0, we can return 0.
		if (number_of_channels < 0)
		{
			number_of_channels = 0; // add 1 to convert from SDLmixer/C-array starts at 0 to Lua arrays start at 1
		...
		setVolume(lua_State *L)
	{
		...
		if ((2 == number_of_arguments) && lua_istable(L, 2))
		{
			...
			ret_flag = sdlAudio.SetVolumeChannel(which_channel, (float)new_volume);
		}
		else
		{
			ret_flag = sdlAudio.SetMasterVolume((float)new_volume);
		...
		getVolume(lua_State *L)
	{
		...
		if ((1 == number_of_arguments) && lua_istable(L, 1))
		{
			...
			current_volume = sdlAudio.GetVolumeChannel(which_channel);
		}
		else
		{
			current_volume = sdlAudio.GetMasterVolume();
		...
		setMaxVolume(lua_State *L)
	{
		...
		ret_flag = sdlAudio.SetMaxVolumeChannel(which_channel, (float)new_volume);
		...
		getMaxVolume(lua_State *L)
	{
		...
		current_volume = sdlAudio.GetMaxVolumeChannel(which_channel);
		...
		setMinVolume(lua_State *L)
	{
		...
		ret_flag = sdlAudio.SetMinVolumeChannel(which_channel, (float)new_volume);
		...
		getMinVolume(lua_State *L)
	{
		...
		current_volume = sdlAudio.GetMinVolumeChannel(which_channel);
		...
		fadeOut(lua_State *L)
	{
		...
		int number_of_channels = sdlAudio.FadeOutChannel(which_channel, number_of_milliseconds);
		// -1 is an error condition. Since the Lua API starts with channel 1 instead of 0, we can return 0.
		if (number_of_channels < 0)
		{
			number_of_channels = 0; // add 1 to convert from SDLmixer/C-array starts at 0 to Lua arrays start at 1
		...
		fade(lua_State *L)
	{
		...
		int number_of_channels = sdlAudio.FadeChannel(which_channel, number_of_milliseconds, (float)new_volume);
		// -1 is an error condition. Since the Lua API starts with channel 1 instead of 0, we can return 0.
		if (number_of_channels < 0)
		{
			number_of_channels = 0; // add 1 to convert from SDLmixer/C-array starts at 0 to Lua arrays start at 1
	...
	static int 	reserveChannels(lua_State *L)
	{
		...
		int return_number_of_channels = sdlAudio.ReserveChannels(number_of_reserve_channels);
		...
		findFreeChannel(lua_State *L)
	{
		...
		if (number_of_arguments > 0)
		{
			start_channel = (int)luaL_checkinteger(L, 1) - 1; // subtract 1 to convert from Lua arrays start at 1 to SDLmixer/C-array starts at 0
		}

		int free_channel = sdlAudio.FindFreeChannel(start_channel);
		...
		{
			free_source = sdlAudio.GetSourceFromChannel(free_channel);
		}

		free_channel++; // add 1 to convert from SDLmixer/C-array starts at 0 to Lua array starts at 1
		...
		getSourceFromChannel(lua_State *L)
	{
		...
		if (number_of_arguments > 0)
		{
			which_channel = (int)luaL_checkinteger(L, 1) - 1; // subtract 1 to convert from Lua arrays start at 1 to SDLmixer/C-array starts at 0
		}

		unsigned int al_source = sdlAudio.GetSourceFromChannel(which_channel);
		...
		getChannelFromSource(lua_State *L)
	{
		...
		int which_channel = sdlAudio.GetChannelFromSource(al_source);
		which_channel++; // add 1 to convert from SDLmixer/C-array starts at 0 to Lua array starts at 1
	...
	{
		/**
		 @fn getDuration(SDLmixer_Data* sound_handle)
		...
		int number_of_arguments = lua_gettop(L);
		int sound_data = 0;
		if (1 == number_of_arguments && lua_isnumber(L, 1))
		{
			sound_data = lua_tointeger(L, 1);
		}
		...
		int total_time = sdlAudio.GetTotalTime(sound_data);
		...
		seek(lua_State *L)
	{
		...
		{
			/*
			ret_val = sdlAudio.RewindChannel(-1);
			if( ret_val <= 0 )
			{
	...
	//		}
			...
			ret_val = sdlAudio.SeekData(sound_data, number_of_milliseconds);
			if (ret_val == 0)
			{
				lua_pushboolean(L, false);
			}
		...
		{
			ret_val = sdlAudio.SeekChannel(which_channel, number_of_milliseconds);
			if (ret_val <= 0)
			{
				lua_pushboolean(L, false);
			}
...
//			}
			...
			ret_val = sdlAudio.RewindData(sound_data);
			if (ret_val == 0)
			{
				lua_pushboolean(L, false);
			}
		...
		{
			ret_val = sdlAudio.RewindChannel(which_channel);
			if (ret_val <= 0)
			{
				lua_pushboolean(L, false);
			}
		...
		isChannelActive(lua_State *L)
	{
		/**
		 ...
		 @endcode
		 */
		int which_channel = (int)luaL_checkinteger(L, 1) - 1; // subtract 1 to convert from Lua arrays start at 1 to SDLmixer/C-array starts at 0;
		if (which_channel < 0)
		{
			lua_pushboolean(L, false);
			return 1;
		...
		int ret_val = sdlAudio.IsActiveChannel(which_channel);
		if (ret_val < 0)
		{
			lua_pushboolean(L, false);
		}
		...
		isSourceActive(lua_State *L)
	{
		...
		int ret_val = sdlAudio.IsActiveSource(al_source);
		if (ret_val < 0)
		{
			lua_pushboolean(L, false);
		}
		...
		isChannelPlaying(lua_State *L)
	{
		...
		int ret_val = sdlAudio.IsPlayingChannel(which_channel);
		if (ret_val < 0)
		{
			lua_pushboolean(L, false);
		}
		...
		isSourcePlaying(lua_State *L)
	{
		...
		int ret_val = sdlAudio.IsPlayingSource(al_source);
		if (ret_val < 0)
		{
			lua_pushboolean(L, false);
		}
		...
		isChannelPaused(lua_State *L)
	{
		...
		int ret_val = sdlAudio.IsPausedChannel(which_channel);
		if (ret_val < 0)
		{
			lua_pushboolean(L, false);
		}
		...
		isSourcePaused(lua_State *L)
	{
		...
		int ret_val = sdlAudio.IsPausedSource(al_source);
		if (ret_val < 0)
		{
			lua_pushboolean(L, false);
		}
		...
		freeChannels(lua_State *L)
	{
		/**
		 ...
		 @endcode
		 */
		int ret_val = sdlAudio.CountAllFreeChannels();
		...
		unreservedFreeChannels(lua_State *L)
	{
		/**
		 ...
		 @endcode
		 */
		int ret_val = sdlAudio.CountUnreservedFreeChannels();
		lua_pushinteger(L, ret_val);
		...
		usedChannels(lua_State *L)
	{
		/**
		 ...
		 @endcode
		 */
		int ret_val = sdlAudio.CountAllUsedChannels();
		lua_pushinteger(L, ret_val);
		...
		unreservedUsedChannels(lua_State *L)
	{
		/**
		 ...
		 @endcode
		 */
		int ret_val = sdlAudio.CountUnreservedUsedChannels();
		lua_pushinteger(L, ret_val);
	...
	static int totalChannels(lua_State *L)
	{
		/**
		 ...
		 @endcode
		 */
		int ret_val = sdlAudio.CountTotalChannels();
		lua_pushinteger(L, ret_val);
		...
		reservedChannels(lua_State *L)
	{
		/**
		 ...
		 @endcode
		 */
		int ret_val = sdlAudio.CountReservedChannels();
		lua_pushinteger(L, ret_val);
	...
	void		LuaLibSDLAudio_Initialize(lua_State *L)
	{
		...
		lua_pop(L, 1); // pop "SDLmixer" table
	}
