...
EmscriptenVideoProvider::EmscriptenVideoProvider(const ResourceHandle<lua_State> & handle, int w, int h)
...
bool EmscriptenVideoProvider::Supports(int source) const
{
	return false;
}
...
bool EmscriptenVideoProvider::Show(int source, lua_State* L, int maxTime, int quality)
{
