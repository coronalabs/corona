...
void
ApplePlatform::HttpPost( const char* url, const char* key, const char* value ) const
{
	if ( Rtt_VERIFY( url && key && value ) )
	{
		NSString *post = [[NSString alloc] initWithFormat:@"%s=%s", key, value];
		...
		Rtt_ASSERT( postData ); // returns nil if there was a lossy conversion

		NSString *postLength = [NSString stringWithFormat:@"%ld", (unsigned long)[postData length]];

		NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
		[request setURL:[NSURL URLWithString:[NSString stringWithExternalString:url]]];
		...
		if ( ! fHttpPostDelegate )
		{
			fHttpPostDelegate = [[Rtt_ConnectionDelegate alloc] init];
		}

		// Probably not a leak because the delegate releases the connection?
		NSURLConnection* c = [[NSURLConnection alloc] initWithRequest:request delegate:fHttpPostDelegate]; Rtt_UNUSED( c );
...
bool
ApplePlatform::HttpDownload( const char* url, Rtt::String &resultData, String& errorMesg, const std::map<std::string, std::string>& headers ) const
{
	resultData.Set("");
	errorMesg.Set("");
...
bool
ApplePlatform::HttpDownloadFile( const char* url, const char *filename, String& errorMesg, const std::map<std::string, std::string>& headers ) const
{
	dispatch_semaphore_t    sem;
	__block bool result = false;
