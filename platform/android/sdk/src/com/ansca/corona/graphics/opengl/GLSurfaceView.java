/*
 * This is a copy of Google's GLSurfaceView class that is normally part of the Android OS.
 * This class was modified by Corona Labs to work-around its bugs and OpenGL driver bugs.
 *
...
public class GLSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        // Do not create the OpenGL surface here.
        // We'll do so in surfaceChanged() which needs to verify that the surface holder is valid.
    }
    ...
    private class ComponentSizeChooser extends BaseConfigChooser {
        public ComponentSizeChooser(int redSize, int greenSize, int blueSize,
                int alphaSize, int depthSize, int stencilSize) {
            super(new int[] {
					// Filter out OpenGL configs that do not meet the given minimum color quality.
                    EGL10.EGL_RED_SIZE, redSize,
                    EGL10.EGL_GREEN_SIZE, greenSize,
                    EGL10.EGL_BLUE_SIZE, blueSize,
					EGL10.EGL_ALPHA_SIZE, 0,
					
					// Filter out OpenGL configs that do not meet the given depth and stencil sizes.
                    EGL10.EGL_DEPTH_SIZE, depthSize,
                    EGL10.EGL_STENCIL_SIZE, stencilSize,
					
					// Filter out software implementations of OpenGL.
					// They are too slow and typically only support OpenGL ES 1.0, which is too old.
					EGL10.EGL_CONFIG_CAVEAT, EGL10.EGL_NONE,
					
					// This must be at the end of the array to tell EGL that there are no more settings.
                    EGL10.EGL_NONE});
            mValue = new int[1];
            mRedSize = redSize;
        ...
        public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display, EGLConfig[] configs) {
			...
			// Traverse all EGL configurations that best fits what the surface object requested.
            for (EGLConfig config : configs) {
				// Skip configurations that do not meet the required depth size.
                int depth = findConfigAttrib(egl, display, config, EGL10.EGL_DEPTH_SIZE, 0);
				if (depth < mDepthSize) {
					continue;
                }
				
				// Skip configurations that do not meet the required stencil size.
                int stencil = findConfigAttrib(egl, display, config, EGL10.EGL_STENCIL_SIZE, 0);
				if (stencil < mStencilSize) {
					continue;
                }
				
				// Fetch the configuration's color channel sizes.
				...
				// Select this EGL configuration if it is near the color size requirements.
				int distance = Math.abs(red - mRedSize) + Math.abs(green - mGreenSize) +
								Math.abs(blue - mBlueSize) + Math.abs(alpha - mAlphaSize);
		...
		private int findConfigAttrib(EGL10 egl, EGLDisplay display, EGLConfig config, int attribute, int defaultValue) {
			mValue[0] = defaultValue;
			try {
				egl.eglGetConfigAttrib(display, config, attribute, mValue);
			}
			catch (Exception e) {
				Log.e("Corona", "Exception was caught in GLSurfaceView.findConfigAttrib", e);
			} 
			return mValue[0];
		}
    ...
    private static class EglHelper {
        ...
        public boolean createSurface() {
            ...
            // Check if the selected EGL configuration supports this device's GPU.
            // Only do this check once. If the retry fails, then we'll fallback to the original configuration.
            if (mHasRetriedCreatingSurface == false) {
                String rendererName = android.opengl.GLES10.glGetString(android.opengl.GLES10.GL_RENDERER);
                if ((rendererName != null) && rendererName.equals("PowerVR SGX 540") &&
                    ...
                    ((ComponentSizeChooser)view.mEGLConfigChooser).setMinAlphaSize(0);
                    boolean wasSurfaceCreated = false;
                    try {
                        wasSurfaceCreated = recreateSurface();
                    }
                    catch (Exception ex) { }
                    if (wasSurfaceCreated == false) {
                        // Failed to create an OpenGL surface that works-around the above bug.
                        // Use the original OpenGL configuration instead.
                        mHasRetriedCreatingSurface = true;
                        ((ComponentSizeChooser)view.mEGLConfigChooser).setMinAlphaSize(previousMinAlphaSize);
        ...
        /**
         * Destroys the last GL surface and then creates a new one using this helper's existing configuration.
         * This is a private method only intended to be used by the createSurface() method.
         */
        private boolean recreateSurface() {
            // Destroy the last GL surface.
            destroySurfaceImp();
            GLSurfaceView view = mGLSurfaceViewWeakRef.get();
            if ((view != null) && (mEglContext != null)) {
                SurfaceHolder holder = view.getHolder();
                if ((holder != null) && (holder.getSurface() != null) && holder.getSurface().isValid()) {
            ...
            // Create and return a new GL surface.
            start();
        ...
        public int swap() {
			...
			// Render the OpenGL content to the display.
                            ...
                            if (view.mNeedsSwap) {
                                ...
                                switch (swapError) {
                                    ...
                                    default:
                                        ...
                                        // TODO: Make this more useful for Corona devs.
                                        Log.i("Corona", "ERROR: Error in drawing our frame! Surface is bad!");
                                        break;
                                }
            ...
            if (! mGLESVersionCheckComplete) {
                ...
                try {
                    ...
                    java.lang.reflect.Method getIntMethod =
                            systemPropertiesClass.getMethod("getInt", new Class[] { String.class, Integer.TYPE });
                    Object value = getIntMethod.invoke(
                            null, new Object[] { "ro.opengles.version", ConfigurationInfo.GL_ES_VERSION_UNDEFINED });
                    mGLESVersion = ((Integer)value).intValue();
                }
                catch (Exception ex) { }
                if (mGLESVersion >= kGLES_20) {
