...
/*
 * Google's helper class that is part of their "Google Play APK Expansion Library".
 * Was modified by Corona Labs to include extra functionality.
 *
...
/**
 * Provides fast access to entries within a zip file.
 * <p>
 * Access to entries is faster than the ZipFile class. Especially for uncompressed entries.
 * <p>
 * This class was written by Google and comes from their "Google Play APK Expansion Library"
 * that is included as an "extra" in their Android SDK. Was modified by Corona Labs to add
...
public class ZipResourceFile {
    ...
    public ZipResourceFile(java.io.File zipFile) throws IOException {
     ...
     * Closes the zip file.
     * <p>
     * Note that it is okay to access the zip file after it has been closed by calling its getter methods.
     * This object will re-open the zip file in this case.
     */
    public void close() {
        // Close all open zip files.
        for (ZipFile zipFile : mZipFiles.values()) {
            if (zipFile != null) {
                try { zipFile.close(); }
                catch (Exception ex) { }
            }
    ...
    public ZipEntryRO getEntry(String assetPath) {
        return mHashMap.get(assetPath);
    }
    ...
    void addPatchFile(String zipFileName) throws IOException
    {
		...
		// "numEntries" is a 16-bit _unsigned_ value so cast it to "char" to preserve unsignedness
        int numEntries = (int) (char) bbuf.getShort(eocdIdx + kEOCDNumEntries);
