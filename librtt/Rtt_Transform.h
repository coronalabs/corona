...
class Transform
{
	...
	public:
		...
		const Matrix& GetMatrix( const Vertex2 *anchorOffset, const Vertex2 *deltas = NULL ) const // <- STEVE CHANGE
		{
			return const_cast< Self* >( this )->GetMatrix( anchorOffset, deltas ); // <- STEVE CHANGE
		}
		Matrix& GetMatrix( const Vertex2 *anchorOffset, const Vertex2 *deltas = NULL ); // <- STEVE CHANGE
